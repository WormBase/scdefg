...
@de_results_tables.route("/", methods=['GET'])
def de_clientside_table_content():
    # convert df to dict for sending as json to datatables
    de_csv=pd.read_csv('results.csv', index_col=0).reset_index()
    # datatables silently breaks (does not display the table) if the json object\
    # that is generated by the jsonify function below is too big
    # NEED TO FIND A SOLUTION! RIGHT NOW ONLY WORKS WITH < 2000 ROWS!!!
    de_csv=de_csv[['gene_name','minuslog10pval','lfc_mean']]
    de_df_nice_names = de_csv.copy().head(1000)
    print(de_df_nice_names.head())
    de_df_nice_names.columns = de_df_nice_names.columns.str.replace('_', ' ')
    de_df_nice_names.columns = de_df_nice_names.columns.str.replace('lfc_mean', 'mean log2 fold change')
    de_df_nice_names.columns = de_df_nice_names.columns.str.replace('-', '&#8209;')
    # convert df to dict for sending as json to datatables
    de_dict_df = de_df_nice_names.to_dict(orient='records')
    # convert column names into dict for sending as json to datatables
    columns = [{"data": item, "title": item} for item in de_df_nice_names.columns]
...
@selection_results_tables.route("/", methods=['GET'])
def selection_clientside_table_content():
    # convert df to dict for sending as json to datatables
    de_csv=pd.read_csv('selected_groups.csv', index_col=0).fillna('-')
    # datatables silently breaks (does not display the table) if the json object\
    # that is generated by the jsonify function below is too big
    # NEED TO FIND A SOLUTION! RIGHT NOW ONLY WORKS WITH < 2000 ROWS!!!
    # de_csv=de_csv[['gene_name','minuslog10pval','lfc_mean']]
    de_df_nice_names = de_csv.copy()
    de_df_nice_names.columns = de_df_nice_names.columns.str.replace('_', ' ')
    de_df_nice_names.columns = de_df_nice_names.columns.str.replace('lfc_mean', 'mean log2 fold change')
    de_df_nice_names.columns = de_df_nice_names.columns.str.replace('-', '&#8209;')
    # convert df to dict for sending as json to datatables
    ...
    de_result_json = jsonify({'data': de_dict_df, 'columns': columns})
    print(de_result_json)
    return de_result_json
