...
def de_clientside_table_content():
    # convert df to dict for sending as json to datatables
    de_csv=pd.read_csv('results.csv', index_col=0)
    ...
    # my hacky solution is to only keep three columns people care about:
    # gene name, p value and log fold change
    # de_csv = de_csv[['minuslog10pval']]
    de_csv=de_csv[['gene_name','minuslog10pval','lfc_mean']]
    de_csv = de_csv.reset_index().head(1000)
    ...
    print(de_csv.head())
    foo = jsonify({'data': de_dict_df, 'columns': columns})
    # foo = jsonify({'data': dict_df, 'columns': columns})
